"""
Logistic Regression with CTGAN Synthetic Data Augmentation
...
print("=" * 80)
print("LOGISTIC REGRESSION WITH CTGAN SYNTHETIC DATA")
...
print(f"  Original training data: {len(train_encoded):,} samples")
print(f"  Synthetic data available: {len(synthetic_data):,} samples")
print(f"  Test data: {len(test_encoded):,} samples")

# Ensure label is 0/1
if train_encoded['label'].min() < 0:
    train_encoded['label'] = train_encoded['label'].replace({-1: 0, 1: 1}).astype(int)
if test_encoded['label'].min() < 0:
    test_encoded['label'] = test_encoded['label'].replace({-1: 0, 1: 1}).astype(int)
...
# ============================================================================
# 2. CALCULATE SYNTHETIC DATA NEEDED FOR 15% CLICKS
# ============================================================================
print("\n[2/6] Calculating synthetic data augmentation...")

target_click_rate = 0.15
# Formula: (n_clicks_original + n_synthetic) / (n_total_original + n_synthetic) = 0.15
# Solving for n_synthetic:
# n_clicks_original + n_synthetic = 0.15 * (n_total_original + n_synthetic)
# n_clicks_original + n_synthetic = 0.15 * n_total_original + 0.15 * n_synthetic
# n_synthetic - 0.15 * n_synthetic = 0.15 * n_total_original - n_clicks_original
# 0.85 * n_synthetic = 0.15 * n_total_original - n_clicks_original
# n_synthetic = (0.15 * n_total_original - n_clicks_original) / 0.85
...
print(f"  Target click rate: {click_rate_augmented*100:.2f}%")

# ============================================================================
# 3. CREATE AUGMENTED TRAINING SET
# ============================================================================
print("\n[3/6] Creating augmented training set...")

# Sample synthetic data (with replacement if needed)
replace = n_synthetic_needed > len(synthetic_data)
synthetic_sample = synthetic_data.sample(n=n_synthetic_needed, random_state=42, replace=replace).copy()
...
non_numeric_cols_baseline = X_full_baseline.select_dtypes(exclude=[np.number]).columns.tolist()
if len(non_numeric_cols_baseline) > 0:
    print(f"  Dropping {len(non_numeric_cols_baseline)} non-numeric columns from baseline")
    X_baseline = X_full_baseline.drop(columns=non_numeric_cols_baseline)
else:
    X_baseline = X_full_baseline
y_baseline = train_encoded['label'].astype(int).copy()

# Augmented model (with synthetic data)
X_full_augmented = train_augmented[feature_cols].copy()
non_numeric_cols_augmented = X_full_augmented.select_dtypes(exclude=[np.number]).columns.tolist()
if len(non_numeric_cols_augmented) > 0:
    X_augmented = X_full_augmented.drop(columns=non_numeric_cols_augmented)
else:
    X_augmented = X_full_augmented
y_augmented = train_augmented['label'].astype(int).copy()

# Test set (no labels available)
X_full_test = test_encoded[test_feature_cols].copy()
non_numeric_cols_test = X_full_test.select_dtypes(exclude=[np.number]).columns.tolist()
if len(non_numeric_cols_test) > 0:
    X_test = X_full_test.drop(columns=non_numeric_cols_test)
else:
    X_test = X_full_test
...
# 6. TRAIN MODELS
# ============================================================================
print("\n[6/6] Training logistic regression models...")
